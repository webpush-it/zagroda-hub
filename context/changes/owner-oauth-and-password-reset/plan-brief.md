...
- Supabase's default for an unverified-email collision is split-brain, not takeover; the plan actively converts that to a block — relies on reading `identity_data.email_verified` correctly per provider.
