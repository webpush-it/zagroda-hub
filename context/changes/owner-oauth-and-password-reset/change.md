...
- Still pending: 3.5, 3.6, and 2.0/2.9 — all gated on the unverified-Facebook collision path. The 2.0 spike's verified→merge and reset-for-OAuth-only behaviors are now confirmed (2.8 + 2.10); only its unverified→separate-user behavior remains, which is exactly the 2.9/3.5 path.
