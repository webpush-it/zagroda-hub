...
- [ ] 2.2 `password_account_exists` denied to anon/authenticated, allowed to service_role
- [ ] 2.3 Type checking passes: `npm run build`
- [ ] 2.4 Linting passes: `npm run lint`
- [ ] 2.5 Unit tests pass incl. `shouldBlockOAuth` truth table: `npm test`
- [ ] 2.6 `src/db/database.types.ts` regenerated with the new function
...
- [ ] 2.10 OAuth-only owner completes reset → can sign in with email+password
