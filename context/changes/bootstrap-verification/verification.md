...
**Post-bootstrap audit shift (after adapter swap):** removing the Cloudflare toolchain cleared its cluster (wrangler/miniflare/ws/yaml). The Vercel adapter introduces a `path-to-regexp` ReDoS chain (`@astrojs/vercel` [direct] → `@vercel/routing-utils` → `path-to-regexp`), adding 3 HIGH. Current `npm audit`: 0 CRITICAL, 4 HIGH, 6 MODERATE, 0 LOW. The HIGH set is `@astrojs/vercel`, `@vercel/routing-utils`, `path-to-regexp` (the Vercel routing chain) plus `devalue` (carried over, transitive). All build/route-tooling, none in the app request path. Not auto-patched.
...
- ~~Swap the Astro adapter from Cloudflare to Vercel.~~ **DONE (2026-05-26):** see the resolved note above. Build verified.
- Address audit findings per your project's risk tolerance — the full breakdown is in this log (note the post-swap shift above).
