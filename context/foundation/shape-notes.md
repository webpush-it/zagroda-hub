...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "główny typ bólu"
      decision: "przerwania w pracy właściciela + koordynacja terminów (anty-overbooking)"
    - topic: "insight"
      decision: "mobile-first dla właściciela pracującego w terenie; polski rynek lokalny jako tło (nie wpływa na konkretne FR-y MVP)"
    - topic: "zasięg persony"
      decision: "jeden właściciel może mieć wiele zagród w panelu"
    - topic: "konto gościa"
      decision: "opcjonalne — formularz publiczny domyślnie, rejestracja po wysłaniu daje historię zapytań"
    - topic: "rejestracja właściciela"
      decision: "otwarta, ale profil zagrody w trybie draft do akceptacji admina (wprowadza rolę admin)"
    - topic: "multi-zagroda w panelu"
      decision: "przełącznik kontekstu 'wybierz zagrodę' — brak agregowanego widoku w MVP"
    - topic: "kanał notyfikacji"
      decision: "email do właściciela przy nowym zapytaniu + email do gościa po akceptacji"
    - topic: "scope-down MVP do 3 tygodni"
      decision: "v1: 1 zagroda/konto, brak moderacji admin, brak konta gościa, reset hasła zostaje; multi-zagroda/moderacja/konto gościa przesunięte do v2"
    - topic: "OAuth dla właściciela"
      decision: "Google + Facebook OAuth jako dodatkowa ścieżka obok email+hasła (obie równoległe); cofnięty z Non-Goals"
    - topic: "weryfikacja emaila w OAuth"
      decision: "email_verified=true od dostawcy = uznajemy za zweryfikowany; FR-006 verification stosuje się tylko do ścieżki email+hasło"
    - topic: "merge kont po emailu"
      decision: "auto-merge tylko gdy dostawca zwraca email_verified=true (anty-takeover guardrail)"
    - topic: "scope OAuth vs 3-tyg. MVP"
      decision: "user świadomie akceptuje potencjalne wydłużenie timeline'u przy dodaniu dwóch dostawców OAuth"
  frs_drafted: 18
  quality_check_status: accepted
---

# Shape Notes — Zagroda Hub

Seed: `idea-notes.md` (Polish). Greenfield. Cel skilla: dojść do struktury, którą `/10x-prd` zamieni na PRD bez wymyślania domeny.
...
Insight: właściciel zagrody pracuje na telefonie w terenie, a nie przy biurku — istniejące SaaS-y bookingowe celują w obsługę z desktopu/recepcji i są nieadekwatne. Drugi insight (kontekstowy, nie load-bearing dla MVP): polski rynek wycieczek edukacyjnych ma swoją specyfikę (sezonowość, dotacje, klasa = ~25 dzieci) — produkt może to dopasować w wersjach następnych.
...
## MVP flow (critical loop)

1. Nauczyciel: katalog publiczny → filtr województwo/miasto → karta zagrody → formularz zapytania (data, liczba uczniów, kontakt) → wyślij.
2. System wysyła email do właściciela: "masz nowe zapytanie".
3. Właściciel: login na mobile → lista oczekujących zapytań → tap na zapytanie → szczegóły + kalendarz.
4. Właściciel: akceptuj. System sprawdza overbooking — przy konflikcie blokuje akcję i pokazuje powód.
5. System wysyła email do nauczyciela: "twoja rezerwacja na <data> w <zagroda> została zaakceptowana".
...
Acknowledged on 2026-05-25: dodanie ścieżek OAuth Google + OAuth Facebook obok email+hasła rozszerza scope ponad pierwotny 3-tygodniowy MVP (OAuth był pierwotnie w Non-Goals z powodu kosztu konfiguracji per-dostawca + obsługi merge'u kont). User świadomie akceptuje, że timeline może wymagać dodatkowych wieczorów/weekendów; `timeline_budget.mvp_weeks: 3` pozostaje jako pierwotny cel, ale przekroczenie nie cofa decyzji o OAuth.

## Open Questions

- **OAuth merge gdy `email_verified=false`** (FR-018): jaki UX gdy dostawca OAuth zwraca emailem nieZweryfikowanym a istnieje już konto email+hasło z tym samym adresem? Bramka anty-takeover blokuje merge — ale czy: (a) login OAuth zostaje odrzucony z komunikatem "potwierdź email u dostawcy i spróbuj ponownie", (b) tworzy się osobne konto OAuth (split-brain), (c) wysyłany jest link weryfikacyjny do email właściciela istniejącego konta z prośbą o ręczne potwierdzenie merge'u. Do rozstrzygnięcia podczas planowania implementacji.
