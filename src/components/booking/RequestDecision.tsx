...
import { Check, CircleAlert, CircleCheck, Loader2, TriangleAlert, X } from "lucide-react";
...
export default function RequestDecision({ id }: Props) {
  ...
  async function decide(action: "accept" | "reject") {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        setNotice({ kind: action === "accept" ? "accepted" : "rejected" });
  ...
  const showButtons = status === "pending" && notice?.kind !== "stale";
